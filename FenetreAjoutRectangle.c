...
static gboolean FenetreAjoutRectangle_change_affichage( GtkComboBox* comboBox, gpointer data );
...
void initialiser_FenetreAjoutRectangle( FenetreAjoutRectangle* fao, Scene* scene )
{
    ...
    gtk_combo_box_append_text( comboBox, "Carré" );
    ...
    g_signal_connect( G_OBJECT( comboBox ), "changed", G_CALLBACK( FenetreAjoutRectangle_change_affichage ), fao );
...
static gboolean FenetreAjoutRectangle_change_affichage( GtkComboBox* comboBox, gpointer data )
{
    FenetreAjoutRectangle* fao = (FenetreAjoutRectangle*)data;

    if( strcmp( gtk_combo_box_get_active_text( comboBox ), "Carré" ) == 0 )
    {
        FenetreAjoutRectangle_enlever_layout( fao );
        ...
        fao->dernierLayout = strcpy( fao->dernierLayout, "Carré" );
...
void FenetreAjoutRectangle_enlever_layout( FenetreAjoutRectangle* fao )
{
    ...
    else if( strcmp( fao->dernierLayout, "Carré" ) == 0 )
    {
        gtk_container_remove( GTK_CONTAINER( fao->layout ), fao->barreSelection );
        gtk_container_remove( GTK_CONTAINER( fao->layout ), fao->barreBouton );
...
static gboolean nouvel_ajout( GtkButton* button, gpointer data )
{
	tdCoord tdCorner1, tdCorner2; /*Coordonnées utilisées pour la création d'un rectangle */
	Rectangle *newRec = NULL; /* Nouveau rectangle*/
    ...
    if( fao->scene->tailleCreation > 0 ) /* Taille renseignée, on crée l'objet */
    {
    	Point_initCoord(tdCorner1, fao->scene->creation->x, fao->scene->creation->y, fao->scene->creation->z);
    	Point_initCoord(tdCorner2, fao->scene->creation->x+longueur, fao->scene->creation->y+largeur, fao->scene->creation->z);
    	newRec = Rectangle_createRectangle(tdCorner1, tdCorner2);
